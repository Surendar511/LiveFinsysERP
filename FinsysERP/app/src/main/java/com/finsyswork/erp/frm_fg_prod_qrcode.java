...
public class frm_fg_prod_qrcode extends AppCompatActivity implements DatePickerDialog.OnDateSetListener{
    ...
    public static Button btn_save,btn_new, btn_cancel, btn_add, btn_scan;
    ...
    public static String reel_wise_lot="N",xtagsca="";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_frm_fg_prod_qrcode);

        this.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_USER_PORTRAIT);

        finapi.context = frm_fg_prod_qrcode.this;
        finapi.deleteJsonResponseFile();

        handler = new sqliteHelperClass(this);
        finapi.setColorOfStatusBar(frm_fg_prod_qrcode.this);

        initializeViews();

        final MyProgressdialog progressDialog = new MyProgressdialog(frm_fg_prod_qrcode.this);
        progressDialog.show();
        new Handler().postDelayed(
                new Runnable() {
                    public void run() {
                        check_form_request();
                        frm_fg_prod_qrcode.xdept = finapi.fill_record_in_listview_popup("EP819");
                        frm_fg_prod_qrcode.xwip_stage = finapi.fill_record_in_listview_popup("EP831");
                        reel_wise_lot = "Y";
        ...
        txttype.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ShowDialogBox(frm_fg_prod_qrcode.this, xtype, "txttype");
        ...
        txtdepartment.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ShowDialogBox(frm_fg_prod_qrcode.this, xdept, "txtdepartment");
                txt_barcode.requestFocus();
            }
        });
        ...
        txtwip_stage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ShowDialogBox(frm_fg_prod_qrcode.this, xwip_stage, "txtwip_stage");
                txt_barcode.requestFocus();
            }
        });
        ...
        txtitem.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                ShowDialogBox(frm_fg_prod_qrcode.this, xitem, "txtitem");
                txt_barcode.requestFocus();
            }
        });
        ...
        btn_new.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                //txttype.setText(frm_fg_prod_qrcode.xtype.get(0));
            }
        });
        ...
        toolbar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(fgen.toolbar_click_count == 5){
                    fgen.showApiName(frm_fg_prod_qrcode.this, ApiName);
                }
            }
        });
        ...
        toolbar.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                finapi.ReadJSONResponse(frm_fg_prod_qrcode.this);
                return false;
            }
        });
        ...
        btn_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                xmv_type = type;
//                if(txttype.getText().toString().isEmpty() && !fgen.frm_request.equals("frm_fg_prod_qrcode"))
//                {
                            ...
                            public void run() {
                                ...
                                switch (fgen.frm_request) {
                                    ...
                                    case "request_material_purchase":
                                        ...
                                        break;
                                    default:
                                        xlast_dt = txtlast_dt.getText().toString();
                                        ApiName="aFGTAG_stack";
                                        for (issue_req_model item : list) {
                                            String litem = (item.getItem().split("---")[0]).toString();
                                            String[] ldept = txtwip_stage.getText().toString().split("---");
                                            String xremarks = txtremarks.getText().toString();
                                            xmv_dep_acd = ldept[0];
                                            xmv_dep_icd = litem;
                                            xmv_dep_req = item.getQty();

                                            post_param += xmbranch + xseperator + "PT" + xseperator + xmv_dep_acd+ xseperator + xmv_dep_icd
                                                    + xseperator
                                                    + xremarks + "!~!~!";
                                        }
                                        break;
                                ...
                                boolean msg = finapi.showAlertMessage(frm_fg_prod_qrcode.this, result);
                                if(!msg){
                                    progressDialog.dismiss();
                                    return;
                                }
        ...
        new Handler().postDelayed(
                new Runnable() {
                    public void run() {
                        ...
                        String codeval_ss ="";
                        String item_ss = txt_barcode.getText().toString().trim();
                        if(item_ss.isEmpty()) {
                            progressDialog.dismiss ( );
                        }
                        else{
                            ...
                            if (fgen.mcd.equals ("GDOT")) {
                                try {
                                    codeval_ss = xicode.substring (0, 17).toString ( );
                                    ArrayList<team6> result = finapi.getApi6 (fgen.mcd, "aFGTAG_check", xicode, fgen.muname, fgen.cdt1.substring (6, 10), "-", "-");
//                                    boolean msg = finapi.RecieveDataFromApi (frm_fg_prod_qrcode.this, x);
                                    Gson  gson = new Gson();
...
//                                        AlertDialog alertDialog = new AlertDialog.Builder (frm_fg_prod_qrcode.this).create ( );
//                                        alertDialog.setTitle ("Error");
//                                        alertDialog.setMessage ("QRCode Already Scan !!");
...
//                                    if (!data.equals("Success"))
//                                     {
//                                       Toast.makeText(getApplicationContext(), "QRCode Already Scan !!", Toast.LENGTH_SHORT).show();
//                                         txtitem.setText("");
//                                         txtqty.setText("");
//                                         txt_barcode.setText("");
//                                         txt_barcode.requestFocus();
...
//                                     }
                            ...
                            if (fgen.xpopup_col1.equals ("Failure")) {
                                AlertDialog alert = new AlertDialog.Builder (frm_fg_prod_qrcode.this).create ( );
                            ...
                            frm_fg_prod_qrcode.txtitem.setText ("" + xtagsca.trim ( ));
//                            frm_fg_prod_qrcode.txtitem.setText ("" + codeval_ss.trim ( ));
                            txt_barcode.setText ("");
                            progressDialog.dismiss ( );
                        }
    ...
    private void check_form_request() {
        fgen.frm_request="prod_entry_gdot";

        switch (fgen.frm_request)
        {
            case "request_material_issue":
                frm_fg_prod_qrcode.xtype = finapi.fill_record_in_listview_popup("EP816B");
                ApiName = "aIssReq_ins";
                xmv_type = "30";
                toolbar.setTitle("Material Issue Request");
                break;
            case "request_material_return":
                frm_fg_prod_qrcode.xtype = finapi.fill_record_in_listview_popup("EP816C");
                xmv_type = "11";
                ApiName = "aissreq_ins";
    ...
    public void ShowDialogBox(Context context, ArrayList<String> list, String controll){
        ...
        if(controll.equals("txtwip_stage")) txtapi_code.setText("EP831");
...
class AsyncCaller_fg_prod_qrcode extends AsyncTask<Void, Void, Void>
{
    @Override
    protected void onPreExecute() {
        super.onPreExecute();
        frm_fg_prod_qrcode.xwip_stage = new ArrayList<>();
